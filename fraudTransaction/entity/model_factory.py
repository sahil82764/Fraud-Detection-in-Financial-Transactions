...
def evaluate_classification_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.6)->MetricInfoArtifact:
    ...
    try:
        ...
        for model in model_list:
            ...
            logging.info(f"{'>>'*20}Started Model Evaluation: [{type(model).__name__}]{'<<'*20}")

            #getting prediction for training and testing dataset
            y_train_pred = model.predict(X_train)
            y_test_pred = model.predict(X_test)

            #calculating r squared error for training and testing dataset
            train_acc = accuracy_score(y_train, y_train_pred)
            test_acc = accuracy_score(y_test, y_test_pred)

            #calculating harmonic mean of train accuracy and test accuracy
            ...
            if metric_info_artifact is None:
                logging.info("No model found with higher accuracy than base_accuracy")
